...
def run(board_count, modification, board_serial_number_list, host_ip):
    ...
    def countdown(t):
        ...
        while t:
            ...
            # self.mysignal_time.emit(f'Идет установка {timeformat}')
            time.sleep(1)
            t -= 1
        # self.mysignal_time.emit('Идет проверка...')
    ...
    def init_disk(connect, sn, stend, place):
        ...
        """
            if 'Product model number: nanoSSD 3ME3' in sata_info:
                check_status = True
                break
            else:
                check_status = False
    ...
    def install_software(connect, wait_time, phase, sn, stend, place):
        ...
        #    raise CustomErrorExtended(['Не удалось начать очистку flash', f'ВЫВОД В КОНСОЛЬ: {console_output}'])
    ...
    def erase_disk(connect, sn, stend, place):
        ...
        if 'Синхронизируются диски' in fdisk_info:
            ...
            logger_script.error(f'При удалении разделов возникла ошибка',
                                extra={'sn': f'{sn}', 'stend': f'{stend}', 'place': f'{place}'})
            # raise CustomErrorExtended (['Ошибка с удалением разделов диска', f'KERNEL PANIC TRACE: (faisk info)', '405'])
    ...
    def clear_ip_pool(stend):
        ...
        try:
            ...
            stdin, stdout, stderr = client.exec_command("clear ip dhep binding *")
    ...
    def hw_check(device):
        ...
        try:
            with Telnet(host_ip, device['port'], timeout=30) as connect:
                ...
                # result[f'device_num_{device_num}']['login_prompt_erase'] = login_to_router(connect)
                # result[f'device_num_{device_num}']['reboot_command'] = send_command(connect, 'system reboot',
                #                                                                    timeout=1, just_wait=True)

                # self.mysignal.emit(f'Вход в Uboot устройства {device_num}...')
                # result[f'device_num_{device_num}']['uboot_prompt_erase'] = enter_uboot(connect, phase)

                # self.mysignal.emit(f'Настройка Bootmenu устройства {device_num} для очистки flash...')
                # result[f'device_num_{device_num}']['bootmenu_1_erase'] = set_bootmenu(connect, host_ip, admin_password, serviceuser_password,
                #                                                                    master_password, phase)

                # self.mysignal.emit(f'Удаление временного ПО с устройства {device_num}...')
                # result[f'device_num_{device_num}']['start_erasing_sw'] = install_software(connect, 40, phase)
            ...
            # logger_debag.debug(traceback.format_exc(), extra={'sn': f'{sn}', 'stend': f'{stend}', 'place': f'{place}'})
