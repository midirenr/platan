...
def set_statistic(stand, date_time):
    Session = sessionmaker(bind=engine)
    db_session = Session()
    statistic = Statistic()
    ...
    statistic.stand = stand
    statistic.date_time = date_time

    db_session.commit()

def get_statistic(stand, date_time):
    name_list = db_session.query(Statistic).filter(
        Statistic.user == user).all()
